...
class MeasurementRepository extends ServiceEntityRepository
{
    ...
    public function findByLocalization(Localization $localization) {
        ...
        $qb->where('m.localization = :localization')
    ...
    public function findByCityName(String $localizationName) {
        $qb = $this->createQueryBuilder('m');

        $qb->where('m.localization.name = :localizationName')
        ->setParameter('localizationName', $localizationName)
        ->andWhere('m.date > :now')
        ->setParameter('now', date('Y-m-d'));
        $query = $qb->getQuery();
        $result = $query->getResult();
